...
// options[prefix] - The prefix used to make the keys unique
...
RedisDocumentStore.prototype.set = function(key, data, callback, skipExpire) {
  var _this = this;
  RedisDocumentStore.client.set(this.prefix + key, data, function(err) {
    if (err) {
      callback(false);
    }
    else {
...
RedisDocumentStore.prototype.setExpiration = function(key) {
  if (this.expire) {
    RedisDocumentStore.client.expire(this.prefix + key, this.expire, function(err) {
      if (err) {
...
RedisDocumentStore.prototype.get = function(key, callback, skipExpire) {
  var _this = this;
  RedisDocumentStore.client.get(this.prefix + key, function(err, reply) {
    if (!err && !skipExpire) {
      _this.setExpiration(key);
    }
